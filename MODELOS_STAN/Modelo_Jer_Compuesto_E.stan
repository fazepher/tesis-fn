...
data{
  ...
  vector[1] m_zeta; // Ocu. Juv.
  vector<lower = 0>[1] s_zeta; // Ocu. Juv.
  vector<lower = 0>[1] sigma_zeta; // Ocu. Juv.
...
model{
  ...
  mu_zeta ~ normal(m_zeta, s_zeta); // Ocu. Juv.
  ...
  for(d in 1:D){
   ...
   zeta[d] ~ normal(mu_zeta,sigma_zeta); // Ocu. Juv.
