...
data{
  ...
  // Composición comunal por la variable explicativa
  matrix<lower=0,upper=1>[C,K] variable;
  
  // Simulaciones MCMC
  vector[D] alfa[M]; // Intercepto
  matrix[D,K] beta_ajus[M]; // Variable
...
generated quantities {
  ...
  // Iniciamos local scope para poder declarar variables intermedias sin que se guarden
  {
    ...
    for(m in 1:M){
      ...
      theta_pred[m,c] = inv_logit(alfa[m,dpto[c]] + variable[c]*(beta_ajus[m,dpto[c]]'));
