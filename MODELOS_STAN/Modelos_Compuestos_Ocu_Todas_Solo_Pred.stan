...
data{
  ...
  matrix<lower=0,upper=1>[C,2] x_ocu_gral; // Composición comunal por Ocu. General
  matrix<lower=0,upper=1>[C,2] x_ocu_juv; // Composición comunal por Ocu. Juvenil
  matrix<lower=0,upper=1>[C,2] x_ocu_may; // Composición comunal por Ocu. Mayores
  ...
  matrix[D,2] zeta_ajus[M]; // Ocu. General
  matrix[D,2] xi_ajus[M]; // Ocu. Juvenil
  matrix[D,2] upsilon_ajus[M]; // Ocu. Mayores
  ...
  // Iniciamos local scope para poder declarar variables intermedias sin que se guarden
  {
    ...
    for(m in 1:M){
      ...
      theta_pred[m,c] = inv_logit(alfa[m,dpto[c]] + 
                                    ...
                                    x_ocu_gral[c]*(zeta_ajus[m,dpto[c]]') + 
                                    x_ocu_juv[c]*(xi_ajus[m,dpto[c]]') + 
                                    x_ocu_may[c]*(upsilon_ajus[m,dpto[c]]'));
